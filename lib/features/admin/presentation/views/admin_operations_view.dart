  ...
  Widget _tabButton(int index, String label, IconData icon) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(border: Border(bottom: BorderSide(color: selected ? MasariColors.primaryCyan : Colors.transparent, width: 3))),
        child: Row(children: [Icon(icon, size: 18, color: selected ? MasariColors.primaryCyan : MasariColors.titaniumLight), const SizedBox(width: 7), Text(label, style: TextStyle(color: selected ? Colors.white : MasariColors.titaniumLight))]),
      ),
    );
  }
    ...
    return MasariCard(
      child: Row(
        children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Text(title, maxLines: 1, overflow: TextOverflow.ellipsis), const SizedBox(height: 4), Text(value, style: MasariTypography.titleMedium(color: color))])),
        ],
      ),
    );
  }

  Widget _security(int users, List<dynamic> audits) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('المستخدمون والتدقيق', style: MasariTypography.headlineSmall(color: Colors.white)),
        const SizedBox(height: 12),
        MasariCard(child: ListTile(leading: const Icon(Icons.people, color: MasariColors.primaryCyan), title: Text('$users حسابات مسجلة'), subtitle: const Text('إدارة المستخدمين الأساسية متاحة من بوابة الإدارة السابقة.'))),
        const SizedBox(height: 12),
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return Padding(
                ...
                child: MasariCard(child: ListTile(leading: const Icon(Icons.history, color: MasariColors.primaryOrange), title: Text(audit.summary), subtitle: Text(audit.timestamp.toString()))),
              );
            },
          ),
        ),
