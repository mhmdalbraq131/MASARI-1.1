...
class _AdminOperationsViewState extends ConsumerState<AdminOperationsView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: MasariColors.darkGraphite, body: SafeArea(child: Column(children: [
      Container(padding: const EdgeInsets.fromLTRB(22, 18, 22, 14), decoration: const BoxDecoration(color: MasariColors.primaryBlue, border: Border(bottom: BorderSide(color: MasariColors.primaryCyan, width: 1.5))), child: Row(children: [
        const Icon(Icons.admin_panel_settings, color: MasariColors.primaryCyan, size: 30), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _dashboard(BuildContext context, int services, int users, int audits) => SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(masariText(context, 'لوحة العمليات', 'Operations Dashboard'), style: MasariTypography.headlineSmall(color: Colors.white)),
    const SizedBox(height: 6), Text(masariText(context, 'مؤشرات حية من نفس الكتالوج الذي يراه العميل.', 'Live indicators from the same catalog used by customer screens.'), style: MasariTypography.bodySmall(color: MasariColors.titaniumGray)),
    const SizedBox(height: 20),
    LayoutBuilder(builder: (context, constraints) { final count = constraints.maxWidth > 850 ? 4 : 2; return GridView.count(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisCount: count, crossAxisSpacing: 14, mainAxisSpacing: 14, childAspectRatio: 2.3, children: [
  ...
  Widget _security(BuildContext context, List<ManagedUser> users, List<dynamic> audits, dynamic admin) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    Expanded(child: ListView(children: [
      ...
      const SizedBox(height: 8), Text(masariText(context, 'سجل التدقيق', 'Audit Log'), style: MasariTypography.titleMedium(color: MasariColors.primaryOrange)), const SizedBox(height: 8),
  ...
  Future<void> _createUser(BuildContext context, dynamic admin) async {
    ...
    await showDialog<void>(context: context, builder: (dialogContext) => StatefulBuilder(builder: (context, setDialogState) => AlertDialog(
      ...
      content: SizedBox(width: 480, child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        DropdownButtonFormField<UserRole>(value: role, decoration: InputDecoration(labelText: masariText(context, 'نوع الحساب', 'Account type')), items: [DropdownMenuItem(value: UserRole.user, child: Text(masariText(context, 'عميل مسافر', 'Traveler'))), DropdownMenuItem(value: UserRole.admin, child: Text(masariText(context, 'مدير نظام', 'Administrator')))], onChanged: (value) => setDialogState(() => role = value ?? role)),
      ])),
      actions: [
