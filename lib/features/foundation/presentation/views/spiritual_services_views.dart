...
class _SpiritualCatalogView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text('الباقات والخدمات المتاحة', style: MasariTypography.titleLarge()),
...
class _ItemCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => MasariCard(child: ListTile(leading: CircleAvatar(backgroundColor: MasariColors.primaryCyan.withValues(alpha: .15), child: const Icon(Icons.workspace_premium, color: MasariColors.primaryCyan)), title: Text(service.name, style: MasariTypography.titleMedium()), subtitle: Text(service.description), trailing: Text('${service.price.toStringAsFixed(0)} ${service.currency}', style: MasariTypography.titleMedium(color: MasariColors.primaryCyan))));
}
