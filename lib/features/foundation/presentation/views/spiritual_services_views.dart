...
class _SpiritualCatalogView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text(masariText(context, 'الباقات والخدمات المتاحة', 'Available packages & services'), style: MasariTypography.titleLarge()), const SizedBox(height: 12),
      if (items.isEmpty) MasariCard(child: Padding(padding: const EdgeInsets.all(22), child: Text(masariText(context, 'لا توجد باقات أو خدمات منشورة حاليًا. يمكن للمدير إضافتها من مركز العمليات.', 'No packages or services are currently published. An administrator can add them from the Operations Center.'))))
...
class _ItemCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) => MasariCard(child: ListTile(
    ...
    trailing: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text('${service.price.toStringAsFixed(0)} ${service.currency}', style: MasariTypography.titleMedium(color: MasariColors.primaryCyan)), TextButton(onPressed: () async { await ref.read(bookingProvider.notifier).createBooking(serviceId: service.id, serviceName: service.name, category: service.category, price: service.price, currency: service.currency); if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(masariText(context, 'تم إنشاء طلب الحجز.', 'Booking request created.')))); }, child: Text(masariText(context, 'احجز', 'Book'))]),
  ));
}

class HajjView extends StatelessWidget { const HajjView({super.key}); @override Widget build(BuildContext context) => _SpiritualCatalogView(title: masariText(context, 'باقات ومجموعات الحج', 'Hajj Packages & Groups'), description: masariText(context, 'باقات الحج والمخيمات والتنقلات التي يديرها فريق مساري.', 'Hajj packages, camps, and transport managed by the MASARI team.'), category: 'حج', icon: Icons.mosque); }
class UmrahView extends StatelessWidget { const UmrahView({super.key}); @override Widget build(BuildContext context) => _SpiritualCatalogView(title: masariText(context, 'برامج العمرة المخصصة', 'Personalized Umrah Programs'), description: masariText(context, 'باقات العمرة الشاملة للتصاريح والطيران والفنادق والإرشاد.', 'Complete Umrah packages covering permits, flights, hotels, and guidance.'), category: 'عمرة', icon: Icons.night_shelter); }
class VisaView extends StatelessWidget { const VisaView({super.key}); @override Widget build(BuildContext context) => _SpiritualCatalogView(title: masariText(context, 'تأشيرات السفر والسياحة والعمرة', 'Travel, Tourism & Umrah Visas'), description: masariText(context, 'خدمات التأشيرات التي ينشرها مديرو العمليات ويحدثون بياناتها.', 'Visa services published and maintained by operations managers.'), category: 'فيزا', icon: Icons.badge); }
