              ...
              child: Column(
                ...
                children: [
                  ...
                  if (session.isAuthenticated) ...[
                    Container(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(children: [
                          ...
                          Text(masariText(context, 'أنت مسجل الدخول حاليًا بحساب:', 'You are currently signed in as:'), style: MasariTypography.titleSmall(color: MasariColors.pureWhite)),
                        ]),
                        ...
                        Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                          MasariPrimaryButton(
                            label: isAdmin ? masariText(context, 'الانتقال إلى بوابة الإدارة', 'Go to Admin Portal') : masariText(context, 'الانتقال إلى الرئيسية', 'Go to Home'),
                            onPressed: () => context.go(isAdmin ? '/admin' : '/home'),
                          ),
                          const SizedBox(height: 8),
                          OutlinedButton(
                            onPressed: () => ref.read(userSessionProvider.notifier).logout(),
                            style: OutlinedButton.styleFrom(foregroundColor: MasariColors.primaryOrange, side: const BorderSide(color: MasariColors.primaryOrange), padding: const EdgeInsets.symmetric(vertical: 12)),
                            child: Text(masariText(context, 'تسجيل الخروج', 'Sign out')),
                          ),
                        ]),
                      ]),
                    ),
                    ...
                    const SizedBox(height: 24),
                    MasariPrimaryButton(
                      label: masariText(context, 'تسجيل الدخول كعميل', 'Sign in as Traveler'),
                      onPressed: () {
                        ref.read(userSessionProvider.notifier).loginAsUser(name: 'أحمد العتيبي', email: _emailController.text);
                        context.go('/home');
                      },
                    ),
                    ...
                    SizedBox(
                      ...
                      child: OutlinedButton.icon(
                        onPressed: () {
                          ref.read(userSessionProvider.notifier).loginAsAdmin(name: 'محمد البراق', email: 'mhmd.albraq@masari.travel');
                          context.go('/admin');
                        },
        ...
        child: SingleChildScrollView(
          ...
          child: Container(
            ...
            child: MasariCard(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 24),
              MasariPrimaryButton(label: masariText(context, 'إنشاء الحساب والمتابعة', 'Create account & continue'), isOrangeCta: true, onPressed: () {
                ref.read(userSessionProvider.notifier).loginAsUser(name: 'سارة الغامدي', email: 'sara@masari.travel');
                context.go('/otp');
              }),
              const SizedBox(height: 12),
      ...
      body: Center(
        child: Container(
          ...
          child: MasariCard(child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 20),
            Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: List.generate(4, (index) => SizedBox(width: 50, height: 50, child: TextField(textAlign: TextAlign.center, keyboardType: TextInputType.number, maxLength: 1, decoration: InputDecoration(counterText: '', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: MasariColors.primaryCyan, width: 2))))),),
            const SizedBox(height: 24),
