...
export const HomeView: React.FC<HomeViewProps> = ({
  ...
  return (
    <div className="p-4 md:p-8 space-y-10 max-w-7xl mx-auto w-full transition-colors">
      {/* Hero Welcome Banner */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0B192C] via-[#1E3E62] to-[#0B192C] text-white p-6 md:p-12 shadow-2xl">
        <div className="absolute top-0 right-0 -mr-20 -mt-20 w-96 h-96 bg-[#00D4FF]/20 rounded-full blur-3xl pointer-events-none"></div>
        <div className="relative z-10 space-y-4 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#00D4FF]/20 border border-[#00D4FF]/30 text-xs font-bold text-[#00D4FF]">
            <Sparkles className="h-4 w-4" />
            <span>{isArabic ? 'منصة مساري الملكية للسفر والسياحة والحج والعمرة' : 'MASARI — Royal Travel & Pilgrimage Platform'}</span>
          </div>
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            <button
              ...
              className="px-6 py-3 rounded-xl bg-[#FF6500] hover:bg-[#EA580C] text-white font-extrabold text-xs shadow-lg transition-all active:scale-95 flex items-center gap-2"
            >
            ...
            <button
              ...
              className="px-6 py-3 rounded-xl bg-[#00D4FF] hover:bg-[#00B4D8] text-[#050914] font-extrabold text-xs shadow-lg transition-all active:scale-95 flex items-center gap-2"
            >
      ...
      {/* Quick Search Widget */}
      <div className="rounded-3xl bg-white dark:bg-[#0A1631] border border-slate-200 dark:border-[#1E293B] p-6 shadow-sm space-y-5">
        {/* Tabs */}
        <div className="flex items-center gap-2 overflow-x-auto border-b border-slate-100 dark:border-slate-800 pb-3">
          {[
            { key: 'flights', labelAr: 'طيران', labelEn: 'Flights', icon: Plane, path: '/flights' },
            { key: 'hotels', labelAr: 'فنادق', labelEn: 'Hotels', icon: Hotel, path: '/hotels' },
            { key: 'tourism', labelAr: 'سياحة', labelEn: 'Tours', icon: Palmtree, path: '/tourism' },
            { key: 'hajj', labelAr: 'حج وعمرة', labelEn: 'Hajj/Umrah', icon: Landmark, path: '/hajj' },
          ].map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                key={tab.key}
                onClick={() => {
                  setActiveSearchTab(tab.key as any);
                }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="md:col-span-3 relative">
            ...
            <input
              ...
              className={`w-full h-13 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 ${
                isArabic ? 'pr-12 pl-4' : 'pl-12 pr-4'
              } text-xs font-bold text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-[#00D4FF]`}
            />
          </div>

          <button
            onClick={() => onNavigate(`/${activeSearchTab}`)}
            className="h-13 w-full rounded-2xl bg-[#FF6500] hover:bg-[#EA580C] text-white font-extrabold text-xs flex items-center justify-center gap-2 shadow-md transition-all active:scale-95"
          >
      ...
      <div className="space-y-4">
        <div>
          <h2 className="text-xl md:text-2xl font-extrabold text-[#0B192C] dark:text-white">
          ...
          {services.map((item) => {
            ...
            return (
              <div
                ...
                className="group cursor-pointer rounded-3xl bg-white dark:bg-[#0A1631] border border-slate-200 dark:border-[#1E293B] hover:border-[#00D4FF]/60 p-5 transition-all duration-300 hover:-translate-y-1 shadow-sm hover:shadow-lg dark:shadow-none flex flex-col justify-between space-y-3"
              >
                ...
                <div>
                  <h3 className="text-sm font-extrabold text-[#0B192C] dark:text-white group-hover:text-[#008DDA] dark:group-hover:text-[#00D4FF] transition-colors">
                ...
                <div className="flex items-center gap-1 text-[11px] font-bold text-[#008DDA] dark:text-[#00D4FF]">
                  <span>{isArabic ? 'حجز فوري' : 'Book Direct'}</span>
                  <ArrowRight className={`h-3 w-3 ${isArabic ? 'rotate-180 group-hover:-translate-x-1' : 'group-hover:translate-x-1'} transition-transform`} />
                </div>
              </div>
            );
          })}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => onNavigate('/tourism')}
            className="text-xs font-bold text-[#008DDA] dark:text-[#00D4FF] hover:underline flex items-center gap-1"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {featuredTours.map((tour) => (
            <div
              key={tour.id}
              onClick={() => onNavigate('/tourism')}
              className="group cursor-pointer rounded-3xl bg-white dark:bg-[#0A1631] border border-slate-200 dark:border-[#1E293B] overflow-hidden shadow-sm hover:shadow-xl dark:shadow-none hover:border-[#00D4FF]/50 transition-all duration-300 flex flex-col justify-between"
            >
              <div className="relative h-48 w-full overflow-hidden bg-slate-200 dark:bg-slate-800">
                ...
                />
                <div className="absolute top-3 right-3 px-2.5 py-1 rounded-xl bg-black/60 backdrop-blur-md text-white text-xs font-bold flex items-center gap-1">
              ...
              <div className="p-5 space-y-3 flex-1 flex flex-col justify-between">
                <div>
                  <h3 className="text-sm font-extrabold text-[#0B192C] dark:text-white leading-snug group-hover:text-[#008DDA] dark:group-hover:text-[#00D4FF] transition-colors">
                ...
                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-base font-black text-[#0B192C] dark:text-[#00D4FF]">
                      {tour.price.toLocaleString()} {isArabic ? tour.currencyAr : tour.currencyEn}
                    </span>
                  </div>
                  <button className="px-3.5 py-2 rounded-xl bg-[#FF6500] text-white font-bold text-xs hover:bg-[#EA580C] shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {featuredPilgrimage.map((pkg) => (
            <div
              ...
              className="group cursor-pointer flex flex-col md:flex-row rounded-3xl bg-white dark:bg-[#0A1631] border border-slate-200 dark:border-[#1E293B] overflow-hidden shadow-sm hover:shadow-xl dark:shadow-none hover:border-[#00D4FF]/50 transition-all"
            >
              <div className="relative md:w-2/5 h-48 md:h-auto overflow-hidden bg-slate-200 dark:bg-slate-800 shrink-0">
                <img
                  src={pkg.image}
                  alt=""
                  className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-500"
                />
                <div className="absolute top-3 right-3 px-2.5 py-1 rounded-xl bg-[#00D4FF] text-[#050914] text-[10px] font-black">
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-1 text-xs text-slate-600 dark:text-slate-400">
                    <div className="flex items-center gap-1.5">
                      <Hotel className="h-3.5 w-3.5 text-[#008DDA] dark:text-[#00D4FF]" />
                      <span className="truncate">{isArabic ? pkg.hotelMakkahAr : pkg.hotelMakkahEn}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Clock className="h-3.5 w-3.5 text-[#008DDA] dark:text-[#00D4FF]" />
                      <span>{pkg.durationDays} {isArabic ? 'يوماً' : 'Days'}</span>
                    </div>
                  </div>
                </div>

                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <div className="text-base font-black text-[#0B192C] dark:text-[#00D4FF]">
                    {pkg.price.toLocaleString()} {isArabic ? 'ر.س' : 'SAR'}
                  </div>
                  <button className="px-4 py-2 rounded-xl bg-[#FF6500] text-white font-bold text-xs hover:bg-[#EA580C]">
      ...
      <div className="rounded-3xl bg-slate-100 dark:bg-[#0A1631] border border-slate-200 dark:border-[#1E293B] p-6 md:p-8 space-y-4">
