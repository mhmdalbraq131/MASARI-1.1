  ...
  return (
    <div className="p-4 md:p-8 space-y-8 max-w-7xl mx-auto w-full transition-colors">
      ...
      {bookingSuccess && selectedTour && (
        <div className="p-4 rounded-2xl bg-[#10B981]/15 border border-[#10B981]/40 text-[#10B981] flex items-center justify-between shadow-lg animate-fadeIn">
          ...
          <button 
            ...
            className="px-3 py-1.5 rounded-xl bg-[#10B981] text-white font-bold text-xs hover:bg-[#059669] transition-colors"
          >
      ...
      <div className="space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className={`w-full h-13 rounded-2xl bg-white dark:bg-[#0A1631] border border-slate-200 dark:border-[#1E293B] ${
              isArabic ? 'pr-12 pl-4' : 'pl-12 pr-4'
            } text-sm text-slate-900 dark:text-[#F8FAFC] placeholder-slate-400 dark:placeholder-[#64748B] focus:outline-none focus:border-[#00D4FF] transition-all shadow-sm`}
          />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPackages.map((tour) => {
          return (
            <div
              key={tour.id}
              className="group flex flex-col rounded-3xl bg-white dark:bg-[#0A1631] border border-slate-200 dark:border-[#1E293B] overflow-hidden shadow-sm hover:shadow-xl dark:shadow-none hover:border-[#00D4FF]/50 transition-all duration-300"
            >
              ...
              <div className="relative h-52 w-full overflow-hidden bg-slate-200 dark:bg-slate-800">
                ...
                {/* Rating Badge */}
                <div className="absolute top-3 right-3 flex items-center gap-1 px-2.5 py-1 rounded-xl bg-black/60 backdrop-blur-md text-white text-xs font-bold border border-white/10">
                  <Star className="h-3.5 w-3.5 text-[#FF6500] fill-[#FF6500]" />
                  <span>{tour.rating}</span>
                  <span className="text-[10px] text-slate-300">({tour.reviewsCount})</span>
                </div>

                {/* Category Tag */}
                <div className="absolute top-3 left-3 px-3 py-1 rounded-xl bg-[#00D4FF] text-[#050914] text-xs font-extrabold shadow-md">
                  {isArabic
              ...
              <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400">
                  ...
                  <div className="flex items-center gap-1.5">
                    <Users className="h-3.5 w-3.5 text-[#008DDA] dark:text-[#00D4FF]" />
                ...
                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3 py-2 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                    >
                      {isArabic ? 'التفاصيل' : 'Details'}
                    </button>
                    <button
                      onClick={() => handleBookNow(tour)}
                      className="px-4 py-2 rounded-xl text-xs font-bold text-white bg-[#FF6500] hover:bg-[#EA580C] shadow-md transition-all duration-200 active:scale-95"
                    >
      ...
      {selectedTour && !bookingSuccess && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#0A1631] border border-slate-200 dark:border-[#1E293B] rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 md:p-8 space-y-6 shadow-2xl">
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <div>
                ...
                <div className="text-2xl font-black text-[#0B192C] dark:text-[#00D4FF]">
              ...
              <div className="flex gap-3">
                ...
                >
                  {isArabic ? 'تأكيد الحجز الفوري' : 'Confirm Instant Booking'}
                </button>
              </div>
            </div>
          </div>
